...
final class TeamViewModel: ObservableObject {
    ...
    func joinTeam(code: String) {
        ...
        if code.isEmpty {
            ...
            db.collection("teams").whereField("members", arrayContains: uid).getDocuments { (querySnapshot, err) in
                if let err = err {
                    print("Error getting documents: \(err)")
                } else {
                    self.setBannerData(title: "Cannot Join a team you're already in!", details: "", type: .warning)
                    self.showBanner = true
                }
            }
